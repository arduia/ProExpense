...
![Date-grouped](../screenshots/screens/journal-list.png)
...
|---|---|---|
| Search field | Keyword/amount search, clearable | `BasicTextField` |
| Filter chip row | Category filter (All + cats + More) | `FilterChip` |
| Day group header | Serif date + mono total | `custom Row` |
| Transaction row | Badge · note · meta · amount · tag | `custom Row in LazyColumn` |
| Bottom sheet — Quick note | Long-press inline note | `ModalBottomSheet` |
...
- filter chip active = ink fill / paper-warm text
- event tag = #FB8C00
- divider line2 rgba(33,33,33,.06)

**Shape · spacing**

- Search field radius 14dp
- filter chip radius 99dp
- row pad 12×8dp, badge 38dp

## Edge cases & error states

### No results

![No results](../screenshots/screens/edge-search.png)

Search with no matches → centered illustration “No matches”, echoes the query, suggests a different keyword/amount/note.

### Long-press quick note

![Long-press quick note](../screenshots/screens/edge-quicknote.png)

Long-press opens a Quick-note sheet pinned to that entry; Save writes the note and dismisses without navigating.
