...
fun FinanceRecord.canonicalPayload(): String {
    ...
    return buildString {
        append("v1")
        append(sep); append(id.value)
        append(sep); append(money.amount.valueInCents)
        append(sep); append(money.currency.code)
        append(sep); append(homeCurrencyMoney.amount.valueInCents)
        append(sep); append(homeCurrencyMoney.currency.code)
        append(sep); append(categoryId.value)
        append(sep); append(type.name)
        append(sep); append(recordedAtEpochMillis)
        append(sep); append(link.canonical())
        append(sep); append(note?.length ?: -1)
        append(sep); append(note ?: "")
    }
}
