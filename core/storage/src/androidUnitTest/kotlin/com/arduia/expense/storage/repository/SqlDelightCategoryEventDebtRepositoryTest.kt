...
class SqlDelightCategoryEventDebtRepositoryTest {
    ...
    @Test
    fun category_observeAll_emitsOrdered() =
        runTest {
            val repo = SqlDelightCategoryRepository(inMemoryDatabase().categoryQueries, Dispatchers.Unconfined)
    ...
    @Test
    fun event_roundTripsBudgetInHomeCurrency() =
        runTest {
    ...
    fun event_upsert_preservesCacheAndCreatedAt_acrossEdit() =
        runTest {
            ...
            val createdAtAfterFirstWrite =
                database.eventQueries
                    .selectEventById("evt-1")
                    .executeAsOne()
                    .created_at
    ...
    @Test
    fun getSpent_returnsCachedTotalInHomeCurrency() =
        runTest {
            val database = inMemoryDatabase()
            val repo = SqlDelightEventRepository(database.eventQueries, Dispatchers.Unconfined)
            repo.upsert(
    ...
    @Test
    fun getSpent_errorWhenEventMissing() =
        runTest {
            val repo = SqlDelightEventRepository(inMemoryDatabase().eventQueries, Dispatchers.Unconfined)
    ...
    @Test
    fun event_observeAll_skipsRowWithUnmappableStatus_insteadOfThrowing() =
        runTest {
            val database = inMemoryDatabase()
            val repo = SqlDelightEventRepository(database.eventQueries, Dispatchers.Unconfined)
            repo.upsert(
