...
class SqlDelightImportExportRepositoryTest {
    ...
    @Test
    fun exportGrouped_returnsOneCsvPerRecordTypeWithRealData() =
        runTest {
            ...
            assertEquals(
                "shared_costs.csv" to "id,title,total_cents,currency_code,participant_count,recorded_at",
                "shared_costs.csv" to files.getValue("shared_costs.csv").lines().first(),
            )
        }

    private suspend fun seedForeignCurrencyRecord(
        repo: SqlDelightImportExportRepository,
    ...
    @Test
    fun csvRoundTrip_recordWithNoNoteAndNoTag_parsesWithoutColumnDesync() =
        runTest {
            val database = inMemoryDatabase()
            val financeRecordRepository =
                SqlDelightFinanceRecordRepository(
    ...
    @Test
    fun csvRoundTrip_foreignCurrencyRecord_preservesHomeCurrencyAmount() =
        runTest {
            val database = inMemoryDatabase()
            val financeRecordRepository =
                SqlDelightFinanceRecordRepository(
    ...
    @Test
    fun jsonRoundTrip_foreignCurrencyRecord_preservesHomeCurrencyAmount() =
        runTest {
            val database = inMemoryDatabase()
            val financeRecordRepository =
                SqlDelightFinanceRecordRepository(
    ...
    @Test
    fun csvRoundTrip_multiLineNote_survivesWithoutDroppingSubsequentRecords() =
        runTest {
            val database = inMemoryDatabase()
            val financeRecordRepository =
                SqlDelightFinanceRecordRepository(
    ...
    @Test
    fun jsonRoundTrip_noteWithQuotesAndNewlines_survivesUnescaped() =
        runTest {
            val database = inMemoryDatabase()
            val financeRecordRepository =
                SqlDelightFinanceRecordRepository(
