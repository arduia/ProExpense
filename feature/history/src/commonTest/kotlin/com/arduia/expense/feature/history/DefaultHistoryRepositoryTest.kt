...
class DefaultHistoryRepositoryTest {
    private class FakeFinanceRecordRepository(
        ...
        override suspend fun getById(id: RecordId): Result<FinanceRecord?> = Result.Success(records.firstOrNull { it.id == id })
        ...
        override suspend fun verifyIntegrity(id: RecordId): Result<Boolean> = Result.Success(records.any { it.id == id })
    ...
    private fun atUtc(
        ...
        day: Int,
        hour: Int = 12,
    ): Long = LocalDateTime(year, month, day, hour, 0).toInstant(TimeZone.UTC).toEpochMilliseconds()
    ...
    @Test
    fun getRecords_filtersByCategoryDateAndQuery() =
        runTest {
            val records =
                listOf(
    ...
    @Test
    fun getSummary_monthly_sumsExpensesInMonthOnly() =
        runTest {
            val records =
                listOf(
    ...
    @Test
    fun getSummary_daily_boundsToAnchorDay() =
        runTest {
            val records =
                listOf(
    ...
    @Test
    fun getSummary_emptyPeriod_returnsZeroInHomeCurrency() =
        runTest {
            val records =
                listOf(
                    record("other-month", atUtc(2026, 7, 1), 300),
    ...
    @Test
    fun getRecordsPage_mapsRecordHistoryFilterOntoPageFilterAndDelegates() =
        runTest {
            val records =
                listOf(
                    record("a", atUtc(2026, 6, 10), 100, category = "food", note = "Coffee shop"),
    ...
    @Test
    fun getRecordsPage_passesCursorThrough() =
        runTest {
            val records =
                listOf(
    ...
    @Test
    fun hasAnyRecordIn_delegatesToExistsByCategory() =
        runTest {
