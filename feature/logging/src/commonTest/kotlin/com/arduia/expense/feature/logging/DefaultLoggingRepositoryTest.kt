...
class DefaultLoggingRepositoryTest {
    private class FakeFinanceRecordRepository(
        ...
        override suspend fun getById(id: RecordId): Result<FinanceRecord?> = Result.Success(stored.value.firstOrNull { it.id == id })
        ...
        override suspend fun verifyIntegrity(id: RecordId): Result<Boolean> = Result.Success(stored.value.any { it.id == id })

        override suspend fun getRecordsPage(
            filter: RecordPageFilter,
            cursor: RecordPageCursor?,
            limit: Int,
        ): Result<List<FinanceRecord>> = Result.Success(stored.value.take(limit))
        ...
        override fun observeChangeSignal() = MutableStateFlow(RecordChangeSignal(stored.value.size.toLong(), 0L))
    }
    ...
    @Test
    fun createRecord_generatesId_andPersists() =
        runTest {
    ...
    @Test
    fun createRecord_propagatesStorageError_withoutFabricatingSuccess() =
        runTest {
    ...
    @Test
    fun deleteRecord_delegatesToContract() =
        runTest {
            val fake = FakeFinanceRecordRepository()
            val repo = DefaultLoggingRepository(fake, idGenerator = { "fixed-id" })
